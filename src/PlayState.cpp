...
void PlayState::createDevConsole() {
    devConsole_screen = sfg::Window::Create(
            sfg::Window::Style::TITLEBAR | sfg::Window::Style::BACKGROUND | sfg::Window::Style::CLOSE);
    ...
    playerDamageAdjustment->GetSignal(sfg::Adjustment::OnChange).Connect(
            std::bind(&PlayState::playerDamageScaleAdjustmentChange, this));
    wallResetAdjustment->GetSignal(sfg::Adjustment::OnChange).Connect(
            std::bind(&PlayState::wallResetAdjustmentChange, this));
    ...
    // the stuff commented out below creates a SFGui warning. I have no idea what it was supposed to do. Also commenting out appears to have no visible effect
